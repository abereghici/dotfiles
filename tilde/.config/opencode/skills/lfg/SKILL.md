...
2. `/ce:plan $ARGUMENTS`
   ...
   GATE: STOP. Verify that the `ce:plan` workflow produced a plan file in `.compound-engineering/docs/plans/`. If no plan file was created, run `/ce:plan $ARGUMENTS` again. Do NOT proceed to step 3 until a written plan exists.
...
5. `/skills:todo-resolve`

6. `/skills:test-browser`

7. `/skills:feature-video`
