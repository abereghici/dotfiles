...
2. `/skills:test-browser` — spawn as background Task agent

Wait for both to complete before continuing.

## Autofix Phase

1. `/ce:review mode:autofix` — run sequentially after the parallel phase so it can safely mutate the checkout, apply `safe_auto` fixes, and emit residual todos for step 7

## Finalize Phase

1. `/skills:todo-resolve` — resolve findings, compound on learnings, clean up completed todos
2. Output `<promise>DONE</promise>` when video is in PR
