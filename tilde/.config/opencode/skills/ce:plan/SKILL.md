...
If the user references an existing plan file or there is an obvious recent matching plan in `.compound-engineering/docs/plans/`:
- Read it
...
Before asking planning questions, search `.compound-engineering/docs/brainstorms/` for files matching `*-requirements.md`.
...
- Institutional learnings from `.compound-engineering/docs/solutions/`
...
- Build the filename following the repository convention: `.compound-engineering/docs/plans/YYYY-MM-DD-NNN-<type>-<descriptive-name>-plan.md`
  - Create `.compound-engineering/docs/plans/` if it does not exist
...
status: active
date: YYYY-MM-DD
origin: .compound-engineering/docs/brainstorms/YYYY-MM-DD-<topic>-requirements.md  # include when planning from a requirements doc
...
- [Relevant `.compound-engineering/docs/solutions/` insight]
...
- **Origin document:** [.compound-engineering/docs/brainstorms/YYYY-MM-DD-<topic>-requirements.md](path)
...
```text
.compound-engineering/docs/plans/YYYY-MM-DD-NNN-<type>-<descriptive-name>-plan.md
```

Confirm:

```text
Plan written to .compound-engineering/docs/plans/[filename]
```
...
**Question:** "Plan ready at `.compound-engineering/docs/plans/YYYY-MM-DD-NNN-<type>-<name>-plan.md`. What would you like to do next?"
...
Based on selection:
- **Open plan in editor** → Open `.compound-engineering/docs/plans/<plan_filename>.md` using the current platform's file-open or editor mechanism (e.g., `open` on macOS, `xdg-open` on Linux, or the IDE's file-open API)
- **`document-review` skill** → Load the `document-review` skill with the plan path
- **Share to Proof** → Upload the plan:
  ```bash
  CONTENT=$(cat .compound-engineering/docs/plans/<plan_filename>.md)
