...
Note that files in `.compound-engineering/docs/plans/` and `.compound-engineering/docs/solutions/` are compound-engineering pipeline artifacts created by `/ce:plan`. They are intentional, permanent living documents — do not recommend their removal or characterize them as unnecessary.
